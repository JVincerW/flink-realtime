...
import com.vincer.app.func.DimAsyncFunction;
import com.vincer.bean.OrderWide;
import com.vincer.bean.PaymentWide;
import com.vincer.bean.ProductStats;
import com.vincer.common.GmallConstant;
import com.vincer.utils.ClickHouseUtil;
import com.vincer.utils.DateTimeUtil;
...
public class ProductStatsApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<ProductStats> clickAndDisplayDS = pageViewDStream.process(new ProcessFunction<String, ProductStats>() {
            @Override
            public void processElement(String pageLog, Context ctx, Collector<ProductStats> out) {
                //转换为JSON对象
        ...
        //3.4 处理下单数据
        SingleOutputStreamOperator<ProductStats> orderDS = orderWideDStream.map((MapFunction<String, ProductStats>) value -> {
            //转换为OrderWide对象
        ...
        //3.7 处理评价数据
        SingleOutputStreamOperator<ProductStats> appraiseDS = commentInfoDStream.map((MapFunction<String, ProductStats>) value -> {
        ...
        //5.设置Watermark
        SingleOutputStreamOperator<ProductStats> productStatsWithWaterMarkDS = unionDS.assignTimestampsAndWatermarks(WatermarkStrategy.<ProductStats>forBoundedOutOfOrderness(Duration.ofSeconds(10L)).withTimestampAssigner((SerializableTimestampAssigner<ProductStats>) (element, recordTimestamp) -> element.getTs()));

        //6.分组、开窗、聚合
        SingleOutputStreamOperator<ProductStats> reduceDS = productStatsWithWaterMarkDS.keyBy(ProductStats::getSku_id)
                .window(TumblingEventTimeWindows.of(Time.seconds(2)))
                .reduce((ReduceFunction<ProductStats>) (stats1, stats2) -> {
